...
bool BNO055I2CNode::readAndPublish()
{
  ...
  double fused_orientation_norm =
      std::pow(std::pow(record.fused_orientation_w, 2) + std::pow(record.fused_orientation_x, 2) +
                   std::pow(record.fused_orientation_y, 2) + std::pow(record.fused_orientation_z, 2),
               0.5);
...
bool BNO055I2CNode::onSrvReset(std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res)
{
  ...
  {
    ROS_ERROR("Exception during reset: %s", e.what());
    res.success = false;
    res.message = e.what();
    return true;
  }

  ROS_INFO("IMU reset succeeded");
  res.success = true;
